...
pid_t fork_exec_pipe(char **args1, char **args2, int fd[2]) {
  ...
  if (pg1 == -1) {                                  // child had to be aborted
    ...
    if (pg2 == -1) {                                // child had to be aborted
      ...
      close(fd[WRITE_END]);                         // no need to write to this pipe now; already done so
...
int main() {
  ...
  char *command_2[] = {"wc", "-l", NULL};
  char *command_3[] = {"figlet", NULL};

  char **pipeline[] = {command_1, command_2, command_3, NULL};

  int result = fork_exec_pipe_ex(command_1, command_2, fd, "in.txt", "out.txt", false);
